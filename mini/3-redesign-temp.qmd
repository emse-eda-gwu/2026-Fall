...
For this assignment, you will use code to redesign the above chart. In this redesign, we are interested in exploring this question:
...
1. Open your course repo (`eda-f26/eda-netID`) in Positron using **File › Open Folder** and open it to the `mini3` folder. If you haven't cloned your course repo yet, follow the steps in [HW1](../hw/1-agentic-workflows-and-github.html#clone-your-course-repo) first.
2. Everything for this assignment lives in the `mini3` folder. Inside it you'll find `report.qmd` and a `data` folder. Click on `report.qmd`. That is the primary file you will edit to conduct your analysis.
3. Update the YAML with your name, a title, the date, etc.
4. Delete any of the existing text / code in `report.qmd` before submitting - it is just there as a helpful guide.

> **Points will be taken off for failing to follow these basic organizing steps.**

Paths inside `report.qmd` are relative to the `mini3` folder, so refer to the data as `file.path('data', 'US_State_Wind_Energy_Facts_2018.xlsx')`.
