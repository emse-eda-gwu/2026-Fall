...
2. Update `_variables.yml`: `title` (year), `crn`, `semester`, `dates`,
   `slack`, `repo`, `site_url`.
3. Update `schedule.csv`: shift every date forward to the new year's actual
   ...
   dates and holidays.
4. Reset each `class/N-*.qmd` to a "Coming soon!" placeholder by swapping
...
5. For each `hw/N-*.qmd`: copy the live file to `hw/N-*-temp.qmd` (this
...
6. Leave `templates/` zip files as-is for now; revise later if that
   assignment changes (see "Update HW template zip files" below).
7. Leave `mini/*.qmd` and their rubrics/templates as last year's content
