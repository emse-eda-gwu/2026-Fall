...
1. Check your email for an invitation to join the `eda-f26` organization on GitHub and **accept it**. Check your spam folder if you don't see it. You can't reach your repo until you've accepted, so do this first. Message Prof. Helveston on Slack and if you can't find it.
2. In GitHub Desktop → **File › Clone Repository** → the **GitHub.com** tab. Your repo will be in the list as `eda-f26/eda-netID`, where `netID` is your GW netID (not your GitHub user name). Select it and click **Clone**.
...
- Ask it what a `.qmd` file does, how is it different from a plain `.R` script, and for a quick demonstration of each. As it to put the demonstration files in the `hw1` folder.
