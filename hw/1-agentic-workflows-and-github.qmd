...
Then, **verify everything works**: open Positron, open the project folder for this assignment (the template you downloaded), open the terminal window in Positron, and run `claude` (or launch a different agent if you're not using Claude Code). If it starts a conversation and asks for your login, it's working.
...
1. GitHub Desktop → **File › New Repository** → name it `eda-practice`.
...
You can check if this folder exists by going to the [list of repos](https://github.com/orgs/eda-f26/repositories) in the course organization GitHub account and searching for your repo, which should be labeled `eda-netID`, where `netID` is your GW netID (not your GitHub user name). If you do not see a repo there, check that you have added your **GitHub username** in [this sheet](https://docs.google.com/spreadsheets/d/13Q4Q9QV28bkGRB2BdGEb2s1zhRrqpnajdyErzIbLS3E/edit?usp=sharing) next to your name
...
In your course repo, open the file `hw1/hw1.qmd` and write at least a paragraph reflecting on your experiences from this assignment. Some prompts you could use:
