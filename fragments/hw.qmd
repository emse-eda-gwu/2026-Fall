...
url_template <- paste0(
  'https://eda.seas.gwu.edu/2026-Fall/templates/hw',
  params$number, '.zip')

hw_file <- paste0('hw', params$number, '.qmd')

review_topic <- get_schedule() %>%
  filter(n_assign == params$number) %>%
  pull(name_class)

review_desc <- get_schedule() %>%
  filter(n_assign == params$number) %>%
  pull(description_class)
```
...
Take a few minutes to review last class, **`r review_topic`**: `r review_desc`

What's one thing from that class you'd want to revisit, or a question you didn't get to ask?
