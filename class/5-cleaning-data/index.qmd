...
wildlife_impacts <- wildlife_impacts %>%
  mutate(
    ...
    phase_of_flt = case_when(
      phase_of_flt %in%
        c('approach', 'arrival', 'descent', 'landing roll') ~ 'arrival',
...
wind <- read_excel(file.path('data', 'US_State_Wind_Energy_Facts_2018.xlsx'))
hot_dogs <- read_excel(file.path('data', 'hot_dog_winners.xlsx'))
```
...
wind <- read_excel(file.path(
  ...
  mutate(
    ...
    `Total Investment ($ Millions)` = as.numeric(
...
hot_dogs <- read_excel(
  file.path('data', 'hot_dog_winners.xlsx'),
  col_types = c(
    'numeric',
    'text',
    'numeric',
...
wildlife_impacts2 <- wildlife_impacts %>%
  mutate(
    season = ifelse(
      incident_month %in% c(3, 4, 5),
      'spring',
      ifelse(
        incident_month %in% c(6, 7, 8),
        'summer',
        ifelse(
          incident_month %in% c(9, 10, 11),
          'fall',
          'winter'
        )
      )
...
# Check out the `lubridate` **[cheat sheet](https://rstudio.github.io/cheatsheets/html/lubridate.html)**
...
wildlife_impacts %>%
  filter(incident_year == 2016, incident_month == 3) %>%
...
- You have **11** days until your [Mini Project 1](https://eda.seas.gwu.edu/2026-Fall/mini/1-data-cleaning.html) is due.
- Look at [HW 4](https://eda.seas.gwu.edu/2026-Fall/hw/4-exploring-data.html), due next week
- Quiz 2 next week 
...
hot_dogs <- read_excel(
  ...
  separate(variable, into = c('variable', 'competition'), sep = '\\.') %>%
...
pc_sales <- read_excel(
  file.path('data', 'pc_sales_2018.xlsx'),
  sheet = 'pc_sales',
  skip = 5
) %>%
  clean_names() %>%
...
pc_sales <- read_excel(
  ...
  separate(year, into = c('drop', 'year'), sep = 'x', convert = TRUE) %>%
  select(-drop)
...
pc_regions <- read_csv(file.path("data", "pc_regions.csv"))
